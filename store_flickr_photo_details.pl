...
if ($gz = gzopen($infile, "rb") ) {
    ...
    while ($gz->gzreadline($buffer) > 0) { $xml .= $buffer; }
...
## http://www.bytemycode.com/snippets/snippet/164/
my $outfile = "x_$infile";

if ($gz = gzopen($outfile, "wb") ) { $buffer =  $doc->toString() ;
  $gz->gzwrite( $buffer ) or die "error writing: $gzerrno\n" ; }
